...
pulumi config set BTT.Iac:dev-ops-vm-td-admin-user-name

# Create and provide azure dev ops agent vm's admin password: (Store it somewhere safe)
pulumi config set BTT.Iac:dev-ops-vm-td-admin-user-password --secret
```
...
pulumi config set BTT.Iac:sql-server-td-db-admin-id
pulumi config set BTT.Iac:sql-server-td-db-admin-password --secret
...
pulumi config set BTT.Iac:sql-server-td-db-user-id
pulumi config set BTT.Iac:sql-server-td-db-user-password --secret

# Provide smtp server's host, port, user, pass and default email sender.
pulumi config set BTT.Iac:default-email-from
pulumi config set BTT.Iac:email-server-host
pulumi config set BTT.Iac:email-server-port
pulumi config set BTT.Iac:email-server-userName
pulumi config set BTT.Iac:email-server-password --secret

# Create and provide jwt secret key
pulumi config set BTT.Iac:jwt-secret-key --secret

# Provide azure dev ops agent vm's IP address which gets created before
pulumi config set BTT.Iac:azure-dev-ops-agent-vm-ip
```
